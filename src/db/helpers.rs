...
pub fn create_call(conn: &PgConnection, chat_id: ChatId, title: &str) -> QueryResult<RollCall> {
    use schema::w_roll_calls::{table, dsl};
    ...
    let close_all_calls = || -> QueryResult<usize> {
        ...
        let update_close = UpdateRollCall::new()
            .with_status(CallStatus::Closed);

        let updated = diesel::update(open_calls)
            .set(update_close)
            .execute(conn)?;
...
pub fn update_title(conn: &PgConnection, chat_id: ChatId, new_title: &str) -> QueryResult<Option<RollCall>> {
    let update = UpdateRollCall::new().with_title(new_title);
    update_call(conn, chat_id, update)
}

pub fn update_quiet(conn: &PgConnection, chat_id: ChatId, quiet: bool) -> QueryResult<Option<CallWithResponses>> {
    let update = UpdateRollCall::new().with_quiet(quiet);
    match update_call(conn, chat_id, update)? {
        None => Ok(None),
        Some(call) => get_responses(conn, call.id).map(|responses| {
            Some((call, responses))
        })
    }
}

fn update_call(conn: &PgConnection,
               chat_id: ChatId,
               update: UpdateRollCall) -> QueryResult<Option<RollCall>> {
    use schema::w_roll_calls::{table, dsl};
...
pub fn get_call_with_responses(conn: &PgConnection, chat_id: ChatId) -> QueryResult<Option<CallWithResponses>> {
...
pub fn set_response(conn: &PgConnection,
                    chat_id: ChatId,
                    user_id: UserId,
                    user_name: &str,
                    attendance: &Attendance) -> QueryResult<Option<CallWithResponses>>
{
...
pub fn set_response_for(conn: &PgConnection,
                        chat_id: ChatId,
                        user_name: &str,
                        attendance: &Attendance) -> QueryResult<Option<CallWithResponses>>
{
...
fn set_response_base<'a, F>(conn: &PgConnection,
                            chat_id: ChatId,
                            value_fn: F) -> QueryResult<Option<CallWithResponses>>
    where F: Fn(CallId) -> NewRollCallResponse<'a>
{
    let open_call = match get_current_call(conn, chat_id)? {
        Some(call) => call,
        None => return Ok(None),
